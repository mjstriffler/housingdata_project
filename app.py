...
engine = create_engine(f'postgresql://{username}:{password}@localhost:5432/housingdata')
...
def home():
    ...
    # 
    state = engine.execute('select "Current residence in" from housingdata', con=engine)
    year = engine.execute('select "Year" from housingdata', con=engine)
    netmigration = engine.execute('select "Net Migration" from housingdata', con=engine)
    unemploymentrate = engine.execute('select "Unemployment Rate" from housingdata', con=engine)
    medianincome = engine.execute('select "Median Income" from housingdata', con=engine)
    # Return template and data
  
   
    return render_template("/templates/tableau.html", state=state, year=year,
    netmigration=netmigration, unemploymentrate=unemploymentrate, medianincome=medianincome)
...
# @app.route("/getMigration/<state_1>/<year_1>")
# def satellite_country(state_1,year_1):

#     # 
#     wanted_state = engine.execute("select * from housingdata where Current residence in='"+state_1+"'", con=engine)
#     chosen_year = engine.execute("select * from hosuing data where Year= '"+year_1+"'", con=engine)
  
#     return render_template("index.html",state=state, year=year, responsedata=responsedata,
#     init_page="initpage", netmigration=netmigration, unemploymentrate=unemploymentrate, medianincome=medianincome)
